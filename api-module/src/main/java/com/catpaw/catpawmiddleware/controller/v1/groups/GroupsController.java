...
public class GroupsController {
    ...
    public ResponseEntity<Result<CustomPageDto<GroupsSummaryDto>>> myGroupSummary(
        ...
        CustomPageDto<GroupsSummaryDto> groupsSummaryList = groupsService.getMyGroupsSummary(memberId, pageable);

        return ResponseEntity
                .ok()
                .body(Result.createPageResult(ResponseCode.SUCCESS.getCode(), null, groupsSummaryList));
    }
    ...
    @ApiResponses({
            ...
            @ApiResponse(responseCode = "500", description = "서버 오류", content = { @Content(schema = @Schema(implementation = Result.class), mediaType = "application/json")})})
    @GetMapping("/summary/me")
    public ResponseEntity<Result<CustomPageDto<GroupsSummaryDto>>> otherGroupSummary(
            @Parameter(hidden = true) @LoginId Optional<Long> idHolder,
            @Parameter(description = "페이지 값 (정렬기준 created 가능)") @PageableDefault(sort = "created", direction = Sort.Direction.DESC) Pageable pageable
    ) {
        if (PageUtils.checkInvalidSort(pageable, PageUtils.CREATED)) {
        ...
        CustomPageDto<GroupsSummaryDto> groupsSummaryList = groupsService.getOtherGroupsSummary(memberId, pageable);
